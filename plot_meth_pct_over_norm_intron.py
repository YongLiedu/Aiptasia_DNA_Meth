...
p = sns.jointplot(np.array(genic_pos), np.array(meth_pct), kind="kde",  
    stat_func=None, color='#64B5CD', size=5, space=0, xlim=(0,1), ylim=(0,100))
# for hex plots
...
p.set_axis_labels('Normalized intron length', 'Methylation %')

if args.savefig:
    fig = plt.gcf()
    
    # without bbox_inches, the saved figure has truncated axes.
    output_filename = args.augmented_bismark_cov.name[:-4] + '.intron.pdf'
