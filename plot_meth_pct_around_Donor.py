...
Uses seaborn to plot a 2D-kde-histogram of the methylation percentage in a
window around the Donor (boundary of exon and intron).""")
...
for row in tsv_reader:
    ...
    # check that the meth positions are in exons but not terminal exon.
    if row[6] not in ['intergenic', 'no_info']:
        if row[10][:4] == 'Exon' and int(row[12]) - int(row[13]) - 1 >= WINDOW:
            if row[10][-3:] != '_-1' and int(row[13]) >= -WINDOW:
                meth_pct.append(float(row[3]))
                genic_pos.append(int(row[13]))

    # part II: get 0 to +WINDOW
    # check that the meth positions are in introns.
    if row[6] not in ['intergenic', 'no_info']:
        if row[10][:6] == 'Intron' and int(row[12]) - int(row[13]) - 1 > WINDOW and int(row[12]) < WINDOW + 2:
            meth_pct.append(float(row[3]))
            genic_pos.append(int(row[12]) - 1)
...
p.set_axis_labels('Distance from donor', 'Methylation %')

if args.savefig:
    fig = plt.gcf()
    
    # without bbox_inches, the saved figure has truncated axes.
    output_filename = args.augmented_bismark_cov.name[:-4] + '.Donor.pdf'
