...
for row in tsv_reader:
    ...
    # check that the meth positions are in intergenic regions
    if row[6] == 'intergenic':
...
p = sns.jointplot(np.array(genic_pos), np.array(meth_pct), kind="kde",  
    stat_func=None, color='#CCB974', size=5, space=0, xlim=(0,1), ylim=(0,100))
# for hex plots
...
p.set_axis_labels('Normalized intergenic distance', 'Methylation %')

if args.savefig:
    fig = plt.gcf()
    
    # without bbox_inches, the saved figure has truncated axes.
    output_filename = args.augmented_bismark_cov.name[:-4] + '.intergenic.pdf'
